 ...
 * VERSION: V1.41
 * * CHANGES:
 * V1.41 - Frontend Trust. Removed evaluateRouteState from saveRoute. The frontend 
 * app.js already correctly manages the difference between 'Ready' and 'Staging'. 
 * saveRoute now blindly trusts and writes the payload.routeState passed from the client, 
 * preventing accidental downgrades immediately after an optimization.
 * V1.40 - Macro State Desync Fix.
 */

const { getField, safeJsonParse } = require('./helpers');

// V1.41 - evaluateRouteState removed. We trust the frontend's evaluation.
...
async function saveRoute(payload, res, db) {
    ...
    if (payload.routeId) {
        ...
        if (driverDoc.exists) {
            // V1.41 - Trust the frontend's routeState
