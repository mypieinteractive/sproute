...
export async function apiFetch(payload) {
    payload.frontEndApiUsage = { 
        geocode: AppState.frontEndApiUsage.geocode, 
        mapLoads: AppState.frontEndApiUsage.mapLoads 
    };
...
export async function loadData() {
    ...
    try {
        ...
        if (data.uploadError) {
            ...
            if (Config.adminParam) {
                apiFetch({ action: 'clearAlert', adminId: Config.adminParam }).catch(e => console.log(e));
            }
            return;
        }
        ...
        if (AppState.stops.length > 0 && AppState.stops[0].eta) {
            AppState.currentStartTime = AppState.stops[0].eta;
        }
...
export function pushToHistory() { 
    ...
    if (AppState.historyStack.length > 20) {
    ...
    if (resurrectedStops.length > 0) {
        ...
        try {
            let payload = { 
                ...
                driverId: Config.isManagerView ? AppState.currentInspectorFilter : Config.driverParam, 
...
export function silentSaveRouteState() {
    ...
    if (routedStops.length === 0) {
        macroState = 'Pending';
    } else if (AppState.dirtyRoutes.has('endpoints_0')) {
        macroState = 'Staging-endpoint'; 
    } else if (AppState.dirtyRoutes.size > 0) {
        macroState = 'Staging';
    }
...
export function getActiveEndpoints() {
    ...
    if (hasRouted && AppState.routeStart && AppState.routeStart.address) {
        start = AppState.routeStart; 
    } else if (insp) {
        start = { address: insp.startAddress || insp.start || '', lat: insp.startLat, lng: insp.startLng };
    }
    
    if (hasRouted && AppState.routeEnd && AppState.routeEnd.address) {
        end = AppState.routeEnd; 
    } else if (insp) {
...
export async function handleGenerateRoute() {
    ...
    try {
        let payload = { 
            ...
            startAddr: eps.start?.address || '', 
            endAddr: eps.end?.address || '', 
            routeState: 'Queued' 
        };
...
export async function handleCalculate() {
    ...
    try {
        ...
        let payload = { 
            action: 'calculate', 
            driverId: Config.isManagerView ? AppState.currentInspectorFilter : Config.driverParam, 
            driver: Config.driverParam, 
            startTime: AppState.currentStartTime, 
            startAddr: eps.start?.address || null, 
            endAddr: eps.end?.address || null, 
            isManager: Config.isManagerView, 
    ...
    try {
        ...
        Array.from(AppState.selectedIds).forEach(id => {
            ...
            if (idx > -1) {
                ...
                AppState.stops[idx].status = 'Pending'; 
                AppState.stops[idx].cluster = 'X'; 
                AppState.stops[idx].manualCluster = false; 
                AppState.stops[idx].eta = ''; 
                AppState.stops[idx].dist = 0; 
                AppState.stops[idx].durationSecs = 0;
...
export async function toggleComplete(e, id) {
    ...
    try {
        let payload = { 
            action: 'updateOrder', 
            rowId: id, 
            driverId: AppState.stops[idx].driverId, 
...
export function sortTable(col) {
    if (AppState.currentSort.col === col) {
        AppState.currentSort.asc = !AppState.currentSort.asc;
    } else { 
        AppState.currentSort.col = col; 
        AppState.currentSort.asc = true; 
    }
...
window.liveClusterUpdate = liveClusterUpdate; 
window.toggleMobileLasso = toggleMobileLasso;
...
document.addEventListener('keydown', (e) => { 
    ...
    if (Config.isManagerView && (e.key === 'Delete' || e.key === 'Backspace')) {
        ...
        if (AppState.selectedIds.size > 0 && AppState.PERMISSION_MODIFY) {
            triggerBulkDelete();
        }
    }
});
...
document.addEventListener('keyup', (e) => { 
    if (e.key === 'Shift') updateShiftCursor(false); 
}); 

document.addEventListener('mousemove', (e) => { 
    updateShiftCursor(e.shiftKey); 
});
