...
/* FILE: app.js */
/* Changes: */
/* 1. Converted to an ES Module Orchestrator. */
/* 2. Abstracted Mapbox logic to map.js and business logic to logic.js. */
/* 3. Consolidated global state into an exported AppState object for UI consumption. */
/* 4. Retained all backend API communication and state history (undo) logic. */
...
// Handle body classes on load
document.body.className = `view-${Config.viewMode} manager-all-inspectors`;
if (Config.isManagerView) document.body.classList.add('empty-state-active');
if (Config.viewMode === 'managermobilesplit') document.body.classList.add('split-show-map');

// Handle basic routing query changes
const currentQuery = window.location.search;
const lastQuery = sessionStorage.getItem('sproute_last_query');
if (lastQuery && currentQuery !== lastQuery && currentQuery.includes('Upload-')) {
    AppState.isFreshGlideRefresh = true;
}
...
initMap(Config.MAPBOX_TOKEN, mapConfig, (event) => {
    ...
    } else if (event.action === 'lasso') {
        event.ids.forEach(id => AppState.selectedIds.add(id));
    }
...
export async function apiFetch(payload) {
    ...
    AppState.frontEndApiUsage.geocode = 0;
    AppState.frontEndApiUsage.mapLoads = 0;
...
export async function loadData() {
    ...
    try {
        ...
        if (data.confirmHijack) {
            ...
            const proceed = await UI.customConfirm(data.message || "The previous admin's session has expired. Take over and overwrite this Inspector's route?");
            UI.showOverlay();
            
            if (proceed) {
                apiFetch({ action: 'executeHijack', adminId: Config.adminParam, driverId: data.driverId || AppState.currentInspectorFilter }).catch(e => console.log(e));
            } else {
                apiFetch({ action: 'cancelHijack', adminId: Config.adminParam, driverId: data.driverId || AppState.currentInspectorFilter }).catch(e => console.log(e));
            }
        ...
        if (AppState.isPollingForRoute) {
            ...
            rawStops.forEach(s => {
                ...
                fetchedMap.set(String(exp.rowId || exp.id), {
                    ...exp, id: exp.rowId || exp.id, status: getStatusText(exp.status),
                    cluster: exp.cluster, manualCluster: false, hiddenInInspector: false,
                    routeState: exp.routeState || s.routeState || globalRouteState,
                    driverId: exp.driverId || s.driverId || globalDriverId,
                    routeTargetId: Config.routeId || null
                });
            });
        ...
        if (!Array.isArray(data)) {
            ...
            UI.applyBranding(data.companyLogo, data.displayName);
            UI.updateInspectorDropdown(); 
            UI.updateRouteButtonColors();
            ...
            if (!hasValidStops && data.companyAddress) {
                ...
                try {
                    ...
                    if (geo.features && geo.features.length > 0) {
                        ...
                        if(map) map.jumpTo({ center: geo.features[0].center, zoom: 11 });
                    }
        ...
        if (!AppState.isPollingForRoute && !AppState.isFreshGlideRefresh) UI.hideOverlay();
        UI.updateUndoUI();
    }
}
...
export function markRouteDirty(driverId, clusterIdx) {
    AppState.dirtyRoutes.add(`${driverId || 'unassigned'}_${clusterIdx || 0}`);
}
...
export function silentSaveRouteState() {
    ...
    if (AppState.dirtyRoutes.has('endpoints_0')) macroState = 'Staging-endpoint';
...
export function getActiveEndpoints() {
    if (!Config.isManagerView) {
        return { 
            start: AppState.routeStart ? { address: AppState.routeStart.address, lat: AppState.routeStart.lat, lng: AppState.routeStart.lng } : null, 
            end: AppState.routeEnd ? { address: AppState.routeEnd.address, lat: AppState.routeEnd.lat, lng: AppState.routeEnd.lng } : null 
        };
    }
    ...
    if (hasRouted && AppState.routeStart && AppState.routeStart.address) start = AppState.routeStart;
    else if (insp) start = { address: insp.startAddress || insp.start || '', lat: insp.startLat, lng: insp.startLng };
    
    if (hasRouted && AppState.routeEnd && AppState.routeEnd.address) end = AppState.routeEnd;
...
export async function handleGenerateRoute() {
    ...
    const hasActiveRoutes = AppState.stops.some(s => isRouteAssigned(s.status));
    ...
    if (isEndpointsDirty) {
        ...
        stopsToOptimize = AppState.stops.filter(s => {
            ...
            return AppState.dirtyRoutes.has(routeKey) || !isRouteAssigned(s.status);
        });
    }
    ...
    try {
        let payload = { 
            action: 'generateRoute', inspectorName: insp.name, driverId: insp.id, 
            stops: flatStopsPayload, startAddr: eps.start?.address || '', endAddr: eps.end?.address || '', routeState: 'Queued' 
        };
        if (!Config.isManagerView) payload.routeId = Config.routeId;
        ...
        if (data.updatedStops || (data.stops && Array.isArray(data.stops))) {
            ...
            AppState.isPollingForRoute = true;
            AppState.pollRetries = 0;
            setTimeout(loadData, 5000);
        } else {
            await loadData();
        }
        ...
        UI.hideOverlay();
        await UI.customAlert("Generation encountered an error. Please wait a moment and try again.");
    } 
}
    ...
    try {
        ...
        if (isEndpointsDirty) {
            ...
            stopsToCalculate = activeStops.filter(s => {
                ...
                return AppState.dirtyRoutes.has(routeKey);
            });
        }
        ...
        let payload = {
            action: 'calculate', driverId: Config.isManagerView ? AppState.currentInspectorFilter : Config.driverParam,
            driver: Config.driverParam, startTime: AppState.currentStartTime, startAddr: eps.start?.address || null,
            endAddr: eps.end?.address || null, isManager: Config.isManagerView,
            stops: stopsToCalculate.map(s => minifyStop(s, s.cluster === 'X' ? 'X' : (s.cluster || 0) + 1))
        };
        if (!Config.isManagerView) payload.routeId = Config.routeId;
        ...
        UI.hideOverlay();
        await UI.customAlert("Error calculating the route. Please try again."); 
    } finally { 
        UI.hideOverlay(); 
    }
}
...
export async function triggerBulkDelete() { 
    ...
    try {
        ...
        UI.hideOverlay();
        await UI.customAlert("Error deleting orders. Please try again.");
    } finally { UI.hideOverlay(); }
}
    ...
    try {
        ...
        Array.from(AppState.selectedIds).forEach(id => {
            ...
            if (idx > -1) {
                ...
                AppState.stops[idx].status = 'Pending'; AppState.stops[idx].cluster = 'X';
                AppState.stops[idx].manualCluster = false; AppState.stops[idx].eta = '';
                AppState.stops[idx].dist = 0; AppState.stops[idx].durationSecs = 0;
        ...
        let payload = { 
            action: 'updateMultipleOrders', updatesList: updatesArray, 
            sharedUpdates: { status: 'P', eta: '', dist: 0, durationSecs: 0, routeNum: 'X' },
            adminId: Config.adminParam 
        };
        if (!Config.isManagerView) payload.routeId = Config.routeId;
        ...
        UI.hideOverlay();
        await UI.customAlert("Error removing orders from the route. Please try again.");
    } finally { UI.hideOverlay(); }
}
...
export async function toggleComplete(e, id) {
    ...
    pushToHistory();
    const idx = AppState.stops.findIndex(s => String(s.id) === String(id));
    const isCurrentlyCompleted = AppState.stops[idx].status.toLowerCase() === 'completed';
    const newStatus = isCurrentlyCompleted ? (AppState.stops[idx].routeState === 'Dispatched' ? 'Dispatched' : 'Routed') : 'Completed';
    AppState.stops[idx].status = newStatus;
    
    triggerFullRender();
    
    try {
        let payload = { 
            action: 'updateOrder', rowId: id, driverId: AppState.stops[idx].driverId, 
            updates: { status: getStatusCode(newStatus) }, adminId: Config.adminParam
        };
        if (!Config.isManagerView) payload.routeId = Config.routeId;
        await apiFetch(payload);
...
export async function performUpload(file, inspectorId, csvType, overrideLock = false) {
    ...
    reader.onload = async (e) => {
        ...
        try {
            ...
            let payload = {
                action: 'uploadCsv', csvData: text, adminId: Config.adminParam,
                driverId: inspectorId, companyId: Config.companyParam || '', csvType: csvType
            };
            if (!Config.isManagerView) payload.routeId = Config.routeId;
            ...
            if (data.success) {
                if (Config.isManagerView && inspectorId && inspectorId !== 'all') {
                    AppState.currentInspectorFilter = String(inspectorId);
                    sessionStorage.setItem('sproute_inspector_filter', AppState.currentInspectorFilter);
                ...
                if (data.unmatchedAddresses && data.unmatchedAddresses.length > 0) {
                    UI.hideOverlay();
                    AppState.unmatchedAddressesQueue = data.unmatchedAddresses;
                ...
                UI.hideOverlay();
                await UI.customAlert("The uploaded file is too large. Please reduce rows and try again.");
            } else if (data.status === 'confirm_hijack') {
                UI.hideOverlay();
                const proceed = await UI.customConfirm(data.message || "This route is currently locked by another admin. Overwrite it?");
                if (proceed) performUpload(file, inspectorId, csvType, true); 
            } else { throw new Error(data.error || "Upload failed"); }
        } catch (err) {
            console.error(err);
            UI.hideOverlay();
            await UI.customAlert("An error occurred during the upload. Please try again.");
        } finally {
            UI.showOverlay("Processing...", "Syncing data with the server");
        }
    };
...
export function setRoutes(num) {
    ...
    // Live cluster logic triggered locally
    const activeStops = AppState.stops.filter(s => isActiveStop(s, Config.isManagerView) && s.lng && s.lat);
...
export function moveSelectedToRoute(cIdx) {
    ...
    AppState.selectedIds.forEach(id => {
        ...
        if (s) {
            ...
            if (hasActiveRoutes) {
                s.status = 'Routed'; s.routeState = 'Staging';
...
// --- Window Bindings for HTML Inline Elements ---
window.AppState = AppState;
window.Config = Config;
window.handleCalculate = handleCalculate;
window.handleGenerateRoute = handleGenerateRoute;
window.handleRestoreOriginal = handleRestoreOriginal;
window.triggerBulkDelete = triggerBulkDelete;
window.triggerBulkUnroute = triggerBulkUnroute;
window.toggleComplete = toggleComplete;
window.undoLastAction = undoLastAction;
window.setRoutes = setRoutes;
window.moveSelectedToRoute = moveSelectedToRoute;
